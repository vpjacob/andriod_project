...
public class SelectShakeDeviceAdapter extends BaseAdapter {
    ...
    public SelectShakeDeviceAdapter(Context mContext, List<AccessDevBean> mDatas) {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (dataBean != null) {
            ...
            holder.checkboxOperateData.setChecked(dataBean.getShakeOpen()==1);

            //注意这里设置的不是onCheckedChangListener，还是值得思考一下的
            holder.checkboxOperateData.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (dataBean.getShakeOpen()==1) {
                        dataBean.setShakeOpen(0);
                    } else {
                        dataBean.setShakeOpen(1);
                    }
                }
            });
        }
